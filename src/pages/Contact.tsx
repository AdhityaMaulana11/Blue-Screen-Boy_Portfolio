  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      ...
      if (devMode) {
        pushLog({
          type: "info",
          msg: `Transmitting message from ${form.name}...`,
        });
  ...
  return (
    <div className="min-h-screen px-4 py-12 sm:py-16">
      <div className="max-w-2xl mx-auto">
        ...
        >
          <WindowFrame
            title={devMode ? "~/sys/contact — transmit" : "~/contact — compose"}
            className={devMode ? "dev-border-glow" : ""}
          >
            <div className="p-5 sm:p-8">
              <TerminalReveal command="nano ~/contact/compose.msg">
                {submitted ? (
                  <div className="text-center py-8 sm:py-10">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {devMode
                        ? "Packet delivered. Awaiting response..."
                  ...
                  <>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                      {[
                        {
                          key: "name",
                          label: devMode ? "sender.name" : "Name",
                          type: "text",
                          placeholder: devMode ? "string" : "Full name",
                        },
                        {
                          key: "email",
                          label: devMode ? "sender.email" : "Email",
                          type: "email",
                          placeholder: devMode
                            ? "string (valid)"
                            : "email@example.com",
                        ...
                        {
                          key: "subject",
                          label: devMode ? "subject" : "Subject",
                          type: "text",
                          placeholder: devMode ? "string | null" : "Optional",
                        },
                      ].map(({ key, label, type, placeholder }) => (
                        <div key={key}>
                          <label
                            className={`text-xs font-mono mb-1 block ${devMode ? "text-terminal-green" : "text-muted-foreground"}`}
                          >
                            {label}
                          </label>
                          <input
